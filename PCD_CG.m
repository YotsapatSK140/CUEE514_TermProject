function [z, history, snr_history] = PCD_CG(A, At, y, lambda, z0, maxIter, x_true)
    % PCD_CG - Proximal Coordinate Descent with Conjugate Gradient for sparse recovery.
    %
    % Solves the optimization problem:
    %   minimize_z 0.5 * ||A*z - y||_2^2 + lambda * ||z||_1
    %
    % This algorithm combines a few steps of Conjugate Gradient (CG) to minimize the smooth
    % part of the objective function (the least squares term) with a proximal step (soft-thresholding)
    % to handle the non-smooth L1 regularization term.
    %
    % Inputs:
    ...
    %   maxIter - Maximum number of outer iterations to run the algorithm.
    %   x_true  - The true underlying signal (optional, for SNR calculation).
    %
    % Outputs:
    %   z           - The estimated solution vector.
    %   history     - A vector containing the objective function value at each outer iteration.
    %   snr_history - A vector containing the Signal-to-Noise Ratio (SNR) at each outer iteration (if x_true is provided).
    ...
    for k = 1:maxIter % Main (outer) iteration loop.
        % Use Conjugate Gradient (CG) to minimize the smooth part: 0.5*||A*z - y||^2
        % The gradient of this smooth part is A'(A*z - y) or At(A(z) - y).
        r = At(y - A(z)); % Initial residual (negative gradient of the smooth part).
        p = r;           % Initial conjugate direction is the residual.
        for j = 1:5  % Perform a small fixed number of inner CG steps.
                     % Limiting the number of CG steps makes this a hybrid approach.
            Ap = At(A(p)); % Apply A and then A' to the conjugate direction.
            alpha = (r' * r) / (p' * Ap + eps);  % Calculate the step size (alpha) for CG.
                                                 % 'eps' is added to the denominator to prevent division by zero.
            ...
            r = r_new;               % Update the residual for the next inner iteration.
        end

        % Proximal step: Apply the soft-thresholding operator to enforce sparsity.
        % This step handles the L1 regularization term.
        z = soft_threshold(z, lambda);

        % Store the objective function value at the end of each outer iteration.
