function [z, history, snr_history] = SSF(A, At, y, lambda, z0, maxIter, x_true)
    % SSF - Iterative Soft Shrinkage-Thresholding Algorithm (ISTA) or Forward-Backward Splitting (FBS) for sparse recovery.
    %
    % Solves the optimization problem:
    %   minimize_z 0.5 * ||A*z - y||_2^2 + lambda * ||z||_1
    %
    % This algorithm is a first-order method that iteratively performs a gradient descent step
    % on the smooth part of the objective function and then applies the proximal operator
    % (soft-thresholding) of the non-smooth part (L1 norm).
    %
    % Inputs:
    ...
    % Estimate the Lipschitz constant (L) of the gradient of the smooth part (0.5*||A*z - y||_2^2).
    % The Lipschitz constant is related to the largest singular value of A squared.
    L = max_singular_value(A, At, randn(size(z0)), 20);
    ...
    for k = 1:maxIter % Main iteration loop.
        % Calculate the gradient of the smooth part: A'(A*z - y).
        grad = At(A(z) - y);

        % Perform a gradient descent step on the smooth part and then apply the soft-thresholding operator.
        % This is the core update rule of the ISTA/FBS algorithm.
        z_new = soft_threshold(z - grad/L, lambda/L);

        % Damping or step size control: Limit the step size to prevent large oscillations or divergence.
        dz = z_new - z;
        if norm(dz) > 1e2 % If the update step is too large
            dz = dz / norm(dz) * 1e2; % Scale it back to a maximum norm of 1e2.
        end
        z = z + dz; % Update the solution.
