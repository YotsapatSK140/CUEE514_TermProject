function [z, history, snr_history] = SSF_SESOP(A, At, y, lambda, z0, maxIter, x_true)
    % SSF_SESOP - Soft Shrinkage-Thresholding Algorithm (SSF) combined with Subspace Expansion by Shifted and Orthogonalized Power method (SESOP) for sparse recovery.
    %
    % Solves the optimization problem:
    %   minimize_z 0.5 * ||A*z - y||_2^2 + lambda * ||z||_1
    %
    % This algorithm enhances the basic SSF/ISTA by incorporating a subspace acceleration technique (SESOP).
    % It takes a standard SSF step and then builds a small subspace of descent directions to potentially
    % find a better update within that subspace, aiming for faster convergence.
    %
    % Inputs:
    ...
    for k = 1:maxIter % Main iteration loop.
        ...
        % Perform the Soft Shrinkage-Thresholding (SSF) step.
        v = soft_threshold(z - grad / L, lambda / L);

        % Build the SESOP subspace by adding the difference between the SSF update and the current solution.
        d = v - z;
        if isempty(D) % If the subspace is empty, initialize it with the first descent direction.
            D = d;
        ...
        % Perform a safety check on the condition number of G to avoid solving ill-conditioned systems.
        if cond(G) < 1e12  % Check if the condition number is below a threshold.
            alpha = -pinv(G) * g;  % Calculate the optimal coefficients for the subspace update using the pseudoinverse for stability.
            z = z + D * alpha;    % Update the solution by moving along the linear combination of the subspace vectors.
        else
            % Fallback strategy: if the subspace system is ill-conditioned, just take the SSF update.
            z = v;
        end
        ...
        % Periodically reset the SESOP directions to prevent the subspace from becoming too large or poorly conditioned.
        if mod(k, 15) == 0
            D = [];
        end
    end
